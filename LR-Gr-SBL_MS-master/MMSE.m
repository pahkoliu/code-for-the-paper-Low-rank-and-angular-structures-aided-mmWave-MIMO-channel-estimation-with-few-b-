function [Zhat, Zvar] = MMSE(Y, Phat,Pvar,Quantizer, Var)
    Delta_min = Quantizer.Delta_min;
    Bit = Quantizer.Bit;
    Stepsize = Quantizer.Stepsize;
%     m = length(Y);
	Mean = 0;
    

    t_lower =  Delta_min + Stepsize*Y;       % lower threshold
    t_upper =  Delta_min + (Y+1)* Stepsize;    % upper threshold

    t_lower(Y == 0) = -inf;
    t_upper(Y == 2^(Bit)-1 ) = +inf;
    ...
    z_lower = real(z_lower);
    z_upper = real(z_upper);
    ...
    if ~isempty(I)
    Zhat(I) = Phat(I) + (Pvar(I) ./ (Pvar(I) + Var + 1/12 * (Stepsize)^2 )).*...
        ( Mean + 0.5*(t_lower(I) + t_upper(I)) - Phat(I));  %% I made a mistake in this line before!!!

    Zvar(I) = Pvar(I).^2 ./ (Pvar(I) + Var + 1/12 * (Stepsize)^2 );
    end;
